...
trait ActiveUserHelper
{
    ...
    {
        $reply_count = Reply::query()->select(DB::raw('user_id,count(*) as reply_count'))
            ->where('created_at','>=',Carbon::now()->subDay($this->pass_days))
            ->groupBy('user_id')
            ->get();

        foreach ($reply_count as $value){
            $reply_sore = $value->reply_count*$this->reply_weight;

            if(isset($this->users[$value->user_id])){
                $this->users[$value->user_id]['score'] += $reply_sore;
            }else{
                $this->users[$value->user_id]['score'] = $reply_sore;
            }
        }
